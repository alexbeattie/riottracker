  ...
  <div class="bg-white shadow rounded-lg p-6 mb-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
      ...
      >
        ...
        <option value="pleaded-not-guilty">
          Pleaded not guilty
        </option>
        <option value="pleaded-guilty">
          Pleaded guilty to one or more charges
        </option>
        <option value="acquitted">
          Acquitted at trial
        </option>
        <option value="convicted">
          Convicted at trial
        </option>
...
export default {
  ...
  methods: {
    ...
    emitFilters() {
      // Create optimized filter payload
      const payload = {
        search: this.filters.searchText.trim(),
        state: this.filters.state,
        charges: this.filters.charges,
        status: this.filters.status,
        affiliations: Object.keys(this.filters.affiliations).filter(
          (k) => this.filters.affiliations[k]
        ),
      };
